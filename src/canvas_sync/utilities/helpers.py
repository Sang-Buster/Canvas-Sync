...
A collection of small static helper-functions used in various modules of Canvas-Sync.
...
def reorganize(items):
    ...
    if (isinstance(items, dict) and list(items.keys())[0] == "errors") or len(
        items
    ) == 0:
        return [], []
...
def get_corrected_name(name):
    ...
    """
    from canvas_sync.utilities import CLEAN_CHARS
...
def validate_token(domain, token):
    ...
    response = str(
        requests.get(
            domain + "/api/v1/courses", headers={"Authorization": "Bearer %s" % token}
        ).text
    )
