...
class Assignment(CanvasEntity):
    ...
    def make_html(self):
        ...
        url = self.settings.domain + "/courses/%s/assignments/%s" % (
            self.get_parent().get_parent().get_id(),
            self.get_id(),
        )

        if not os.path.exists(self.sync_path + self.name + ".html"):
            with io.open(
                self.sync_path + self.name + ".html", "w", encoding="utf-8"
            ) as out_file:
                out_file.write("<h1><strong>%s</strong></h1>" % self.name)
                out_file.write(
                    '<big><a href="%s">Click here to '
                    "open the live page in Canvas</a></big>" % url
                )
                out_file.write("<hr>")
                out_file.write(
                    self.assignment_info.get("description") or "No description"
                )
