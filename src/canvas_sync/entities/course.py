...
from canvas_sync.entities.assignments_folder import AssignmentsFolder
...
class Course(CanvasEntity):
    def __init__(self, course_info, parent, settings):
        ...
        course_name = helpers.get_corrected_name(
            self.course_info["course_code"].split(";")[-1]
        )
        ...
        self.to_be_synced = (
            True if course_name in parent.settings.courses_to_sync else False
        )
