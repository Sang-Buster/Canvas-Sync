...
from canvas_sync.entities.external_url import ExternalUrl
...
class Module(CanvasEntity):
    def __init__(self, module_info, module_position, parent, identifier="module"):
        ...
        module_path = os.path.join(
            parent.get_path(), "%s - %s" % (module_position, module_name)
        )
    ...
    def add_sub_header(self, folder_info, folder_position, folder_items):
        ...
        from canvas_sync.entities.sub_header import SubHeader

        sub_folder = SubHeader(
            folder_info, folder_position, parent=self, items=folder_items
        )
    ...
    def add_items(self, items=None):
        ...
        for item in items_in_this_scope:
            if item["type"] == "File" and self.settings.modules_settings["Files"]:
                ...
                item["type"] == "Page" and self.settings.modules_settings["HTML pages"]
            ):
                self.add_page(item)
            elif (
                item["type"] == "ExternalUrl"
                and self.settings.modules_settings["External URLs"]
            ):
                self.add_url(item)
