...
class Page(CanvasEntity):
    def __init__(self, page_info, parent):
        ...
        self.page_info = (
            self.page_item_info if "id" not in self.page_item_info else None
        )

        page_id = (
            self.page_item_info["id"]
            if not self.page_info
            else self.page_info["page_id"]
        )
    ...
    def download(self):
        ...
        self.page_info = (
            self.api.download_item_information(self.page_item_info["url"])
            if not self.page_info
            else self.page_info
        )

        # Create a HTML page locally and add a link leading to the live version
        body = self.page_info.get("body", "")
        html_url = self.page_info.get("html_url", "")
        ...
        if not os.path.exists(self.sync_path):
            with io.open(self.sync_path + ".html", "w", encoding="utf-8") as out_file:
                out_file.write("<h1><strong>%s</strong></h1>" % self.name)
                out_file.write(
                    '<big><a href="%s">Click here to open the live page in Canvas</a></big>'
                    % html_url
                )
                out_file.write("<hr>")
                out_file.write(body)
